...
namespace MyFinances.Data
{
    public class DebentureService
    {
        ...
        private void CalculateCOI(Debenture debentureResult)
        {
            ...
            var earlyRedemptionFee = DebentureModel.EarlyRedemptionFee;
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                totalProfitRes[i] = totalProfit[i] - earlyRedemptionFee * DebentureModel.Amount > 0 && i < 4 ? totalProfit[i] - earlyRedemptionFee * DebentureModel.Amount : totalProfit[i];
            }
        ...
        private void CalculateEDO(Debenture debentureResult)
        {
            ...
            double earlyRedemptionFee = DebentureModel.EarlyRedemptionFee;
        ...
        private void CalculateTOS(Debenture debentureResult)
        {
            ...
            var earleRedemptionFee = DebentureModel.EarlyRedemptionFee;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (DebentureModel.BelkaTax)
                {
                    calculatedTax = (Math.Ceiling(Math.Max(totalProfit[i] / DebentureModel.Amount - earleRedemptionFee, 0) * 19)) / 100 * DebentureModel.Amount;
                    if (i == 3)
                        calculatedTax = Math.Ceiling(totalProfit[i] / DebentureModel.Amount * 19) / 100 * DebentureModel.Amount;
                }

                totalProfitRes[i] = totalProfit[i] - calculatedTax;
                totalProfitRes[i] = i != 3 && i != 0 ? totalProfitRes[i] - earleRedemptionFee * DebentureModel.Amount : totalProfitRes[i];
